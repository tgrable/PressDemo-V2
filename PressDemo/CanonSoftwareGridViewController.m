...
-(void)appCameBackIntoFocus
{
    ...
    if ([model.hostReachability isReachableViaWiFi]) {
        ...
        [model logData:@"CanonSoftwareGridViewController" withAction:@"No Internet Connection" withLabel:@"Came back into focus with no connection"];
    }
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if ([model.hostReachability isReachableViaWiFi]) {
        ...
        noConnection.alpha = 1.0;
        [model logData:@"CanonSoftwareGridViewController" withAction:@"No Internet Connection" withLabel:@"Running the app with no internet"];
    }
}
