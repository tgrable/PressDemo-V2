...
-(void)appCameBackIntoFocus
{
    ...
    if ([model.hostReachability isReachable]) {
        ...
        [model logData:@"SeriesViewController" withAction:@"No Internet Connection" withLabel:@"Came back into focus with no connection"];
    }
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if ([model.hostReachability isReachableViaWiFi]) {
        ...
        noConnection.alpha = 1.0;
        [model logData:@"SeriesViewController" withAction:@"No Internet Connection" withLabel:@"Running the app with no internet"];
    }
}
